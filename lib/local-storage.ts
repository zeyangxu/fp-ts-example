...
export function getStorageItem<T>(type: t.Mixed, key: string, storage = localStorage) {
  return function (onNone: Lazy<void>, onSome: (data: T) => void) {
    option.fold(
      onNone,
      onSome
    )(parse<T>(type, storage.getItem(key)))
  }
}

export function setStorageItem<T>(type: t.Mixed, key: string, storage = localStorage) {

...
export function stringify<A>(type: t.Mixed, val: A, serializer: Serializer = JSON): option.Option<string> {
  return pipe(
    option.fromEither(type.decode(val)),
    option.chain(
      (decoded) => option.tryCatch(() => {
...
export function parse<A>(type: t.Mixed, val: string | null, serializer: Serializer = JSON): option.Option<A> {
  return pipe(
    ...
    option.chain((parsed) => {
      return option.fromEither(type.decode(parsed) as either.Either<t.Errors, A>)
    })
  )
}
